...
# RESULTS
# ---------------------------------------------------------------------------------
# Long: Dev.ROI >= 1.10; Val.ROI >= 1.10
# Shrt: Dev.ROI >= 1.10; Val.ROI >= 1.10
# BRW ROI: 2.3 - 3.9 - 6.0 @ 10 picks
# XXX ROI: 1.86X
# ---------------------------------------------------------------------------------
# Long: Dev.ROI >= 1.10; Val.ROI >= 1.01
# Shrt: Dev.ROI >= 1.10; Val.ROI >= 1.01
# BRW ROI: 1.8 - 7.4 - 13.6 picks
# XXX ROI: 1.64X
# ---------------------------------------------------------------------------------

Parms <- list(algoIds = c("20191006")
...
source("./Functions/20191006.Trading.Simulation.R")
...
rm(stocks)
rm(list = lsf.str())
saveRDS(sum, paste0(Parms[["summary.folder"]], "20191006.Performance.Summary.rds"))
# sum <- readRDS(paste0(Parms[["summary.folder"]], "20191006.Performance.Summary.rds"))
...
saveRDS(hist.perf, paste0(Parms[["summary.folder"]], "20191006.Performance.Detail.rds"))
# hist.perf <- readRDS(paste0(Parms[["summary.folder"]], "20191006.Performance.Detail.rds"))
rm(stocks)

# -------------------------------------------------------------------------
x1 <- anti_join(sum
                , sum %>%
                  filter(ROI.Dev > 1, ROI.Val > 1) %>%
                  group_by(ticker) %>%
                  summarise(ROI.XXX = prod(ROI.XXX^Trades.XXX, na.rm = TRUE)) %>%
                  filter(ROI.XXX < 1) %>% select(ticker)
                , by = "ticker")

sum.select <- x1 %>%
              filter((Type == "SHRT" & ROI.Dev >= 1.1 & ROI.Val >= 1.1) |
                       (Type == "LONG" & ROI.Dev >= 1.1 & ROI.Val >= 1.1)) %>%
              select(ticker, ID, R)
...
openxlsx::write.xlsx(list("T9.Summary" = T9.Summary), file = "./Reports/20191006 Type 9.xlsx")
rm(list = lsf.str())
...
prod.models <- inner_join(sum, sum.select, by = c("ticker", "ID", "R")) %>%
                mutate(algoId = "20191006") %>%
                  select(algoId, ticker, ID, Type, R, R.low, R.high, R.buy, R.sell, R.stop)  

saveRDS(prod.models, paste0(Parms[["summary.folder"]], "20191006.Production.Models.rds"))
