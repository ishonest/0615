...
Dev.Simulation <- function(ticker, validation.period = 120)
{
  ...
  picked <- all %>% filter(ds <= Parms[["last.dev.date"]]) %>%
            ...
            # filter(ID == 160) %>%
            summarise(av.l = mean(ROI.l), sd.l = sd(ROI.l),
                      av.h = mean(ROI.h), sd.h = sd(ROI.h)) %>%
            mutate(buy = av.l - sd.l,
                   sell = av.h + sd.h,
                   R.ROI = sell/buy,
                   L.stop = pmax(+ 1 - (av.l - 3*sd.l)/buy, 0.04),
                   S.stop = pmax(- 1 + (av.h + 3*sd.h)/sell, 0.04),
                   L.ROR = (R.ROI - 1)/L.stop,
                   S.ROR = (R.ROI - 1)/S.stop,
                   Type = case_when(sd.l < sd.h ~ "LONG",
                                    sd.l > sd.h ~ "SHRT")
                  ) %>%
                filter(!is.na(Type)) %>%
                mutate(R.buy = ifelse(Type == "LONG", buy, sell),
                       R.sell = ifelse(Type == "LONG", sell, buy),
                       R.stop = ifelse(Type == "LONG", L.stop, S.stop),
                       R.ROR = ifelse(Type == "LONG", L.ROR, S.ROR),
                       ) %>%
  ...
  {
    rm(ticker, d1, all, picked, validation.period)
    return(NULL)
  }
  ...
  sum <- Simulated.Performance.Model(sim, validation.period)

  # x <- sum %>% filter(ROI.Val >= ROI.Dev - 0.05, ROI.Val <= ROI.Dev + 0.05, ROI.Dev >= 1.1)
  ...
  rm(ticker, d1, all, picked, sim, validation.period)
  return(sum)
}
