...
Dev.Simulation <- function(ticker, validation.period = 120)
{
  ...
  val.models <- all %>%
                ...
                summarise(N = n(), M = mean(ROI.c, na.rm = TRUE)) %>% 
                data.table::setDT() %>%
                ...
                group_by(ID) %>%
                arrange(ticker, ID, -R) %>%
                mutate(Type = case_when(M.Dev <= 0.95 & M.Val <= M.Dev + 0.02 ~ "SHRT",
                                        M.Dev >= 1.05 & M.Val >= M.Dev - 0.02 ~ "LONG")) %>%
                filter(!is.na(Type)
                       , between(N.Val, 0.618*validation.period/10, 1.618*validation.period/10)) %>%
                ungroup() %>% arrange(ticker, ID, R)
  
  if(is.null(val.models) || nrow(val.models) == 0)
  {
    rm(ticker, d1, all, validation.period, val.models)
    return(NULL)
  }
  ...
  {
    rm(ticker, d1, all, validation.period, val.models, picked)
    return(NULL)
  }
  ...
  rm(ticker, d1, all, validation.period, val.models, picked, sim)
  return(sum)
}
