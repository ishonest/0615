...
Parms <- list(algoIds = c("20190802", "20190813")
...
saveRDS(prod.models, paste0(Parms$data.folder, "Summary/Production.Models.rds"))
                   ...
                   ) %dopar%
{
          ...
          foreach(model.ID = unique(all$ID), .combine = bind_rows, .errorhandling = 'remove') %do%
  {
    ...
    # algo.ID = Parms$algoIds[2]
    # model.ID = unique(all$ID)[1]
  ...
  can.buy <- sim %>% group_by(ticker, ds) %>%
              ...
              filter(ds == max(ds)) %>% select(BOD.in.hand) %>%
              unlist() %>% as.numeric()
  ...
  {
      sim <- sim %>% 
              # # Get Rid of 0s post stop loss
              mutate(buy.price = case_when(buy.price > 0 ~ buy.price)) %>%
              # # Select today's targets
              filter(ds == max(ds)) %>%
              filter(!is.na(buy.price) | (!is.na(sell.price) & !is.na(action)) ) %>%
              # # Filter within capacity only
              group_by(action) %>% mutate(rank = rank(-buy.price, ties.method = "min")) %>%
  ...
  } else
  {
    ...
    today <- sim %>% group_by(ticker, ID, algoId) %>% filter(ds == max(ds)) %>%
              select(algoId, ticker, ds, buy.price, sell.price, stop.price, last.sell, action, invest, 
...
# -------------------------------------------------------------------------
do.call(file.remove,
        list(list.files(paste0(Parms$data.folder
                               , c("Process.Tracker/", "Production/Scores/", "Production/Simulation/"))
                        , full.names = TRUE)))
