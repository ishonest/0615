          ...
          sim <- foreach(i = unique(val.models$ID), .combine = bind_rows, .errorhandling = 'remove') %do%
                  {
                    ...
                    sim <- left_join(all.ID, picks, by = c("ticker", "ID", "R", "R.low", "R.high")) %>%
                            ...
                            mutate(sell.window = ifelse(!is.na(lag(buy.window)) | !is.na(buy.window), 1, NA)
                                   ...
                                   # Prorated Sell Price: 90% - 98% in Day 1-5
                                   , sell.price = R.sell*lag(close)
                                   , sell.price = sell.window*zoo::na.locf(sell.price, na.rm = FALSE)
                                   , sell.price = case_when(signal <= 5 ~ (0.88 + signal*0.02)*sell.price,
                                                            signal >  5 ~ sell.price)
...
saveRDS(hist.perf, paste0(data.folder, "Summary/20190822.Historical.Performance.rds"))
rm(stocks)
...
openxlsx::write.xlsx(list("T9.Summary" = T9.Summary), file = "./Reports/20190822 Type 9.xlsx")
...
saveRDS(prod.models, paste0(data.folder, "Summary/20190822.Production.Models.rds"))
